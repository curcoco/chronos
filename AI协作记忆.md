...
- **屏幕时间防沉迷（同日追加，健康页第 5 Tab，DB v18）**：读系统 UsageStats（原生 `ScreenTimePlugin.kt` 通道，无后台监控，打开时同步近 8 天）；app 三级分类（用户覆盖 > 内置娱乐预设约 28 个 > 默认工具）；今日娱乐/预算卡（超预算变红）+ 近一年 GitHub 式热力图 + 今日娱乐排行 + **近7天娱乐趋势弹层（排行卡右上入口，堆叠柱+点天看明细；用户要求详细数据点开才展示）** + 分类管理页；金币联动（昨日达标发 2，type `screen`）；预算存 `screenBudgetMinutes`（默认 120 分钟）。**共 94 项测试全过**（新增 10），`dart analyze` 零告警。
